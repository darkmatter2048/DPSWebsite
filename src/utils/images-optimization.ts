...
import { probeRemoteImage } from './remote-image-probe';
...
export const unpicOptimizer: ImagesOptimizer = async (image, breakpoints, width, height, format = undefined) => {
  ...
  // 如果没有提供宽度或高度，尝试使用我们的自定义函数获取远程图片尺寸
  if ((!width || !height) && image.startsWith('http')) {
    try {
      const dimensions = await probeRemoteImage(image);
      if (dimensions) {
        width = width || dimensions.width;
        height = height || dimensions.height;
      }
    } catch (error) {
      console.warn('获取远程图片尺寸失败:', error);
      // 使用默认尺寸
      width = width || 1200;
      height = height || 630;
    }
